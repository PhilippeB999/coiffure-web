...
const STORAGE_KEY = "coiffurequest_state_v1";
...
const ACCESS_CODE = "COIFFURE-2026-RJW2"; // code fourni au centre de formation après achat de la licence
...
const SYNC_KEY = "coiffurequest_sync_pending";
...
function deviceId() {
  let id = localStorage.getItem("coiffurequest_device_id");
  if (!id) {
    id = "cq-" + (crypto.randomUUID ? crypto.randomUUID() : Date.now().toString(36) + Math.random().toString(36).slice(2));
    localStorage.setItem("coiffurequest_device_id", id);
  }
  return id;
}
